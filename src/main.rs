...
use crate::layer::{create_diff_layer, create_filtered_layer, extract_from_layer, load_image_layer, AbsLayer};
use color_manipulator_rust::{add, add_list, color, filter_color, filter_scalar, filter_scalar_and_stretch, get_b, get_distance, get_g, get_r, gradient_linear, mult, only_b, only_g, only_r, safe_color, scalar, sub, xor_color, zero_but_one_in_square, POS, RGB};
...
fn img_20241009_161110() {
    create_filtered_layer(
        ...
        |c, p| {
            ...
            let quad_xist = zero_but_one_in_square(
                p,
                POS { x: 0.42, y: 0.05 },
                POS { x: 0.6, y: 0.2 },
            );
            let quad_istanbul = zero_but_one_in_square(
                p,
                POS { x: 0.28, y: 0.7 },
                POS { x: 0.79, y: 0.85 },
            );
            ...
            return add_list(&[
                Some(
                    mult(
                        mult(
                            scalar(riflessi_cerchi * pomello_center),
                            bg_color,
                        ),
